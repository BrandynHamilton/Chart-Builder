...
try:
    from utils import colors, clean_values, clean_values_dollars, ranked_cleaning, to_percentage, rank_by_col, rank_by_columns, normalize_to_percent, data_processing, top_other_ts_by_col, top_other_ts_by_columns, top_ts_by_col, top_ts_only_by_columns, cleaning, cleaning_values, top_by_col, top_other_by_col,to_time
    from plots import simple_bar_plot, simple_line_plot, sorted_bar_chart, sorted_multi_line, ranked_bar_chart, line_and_bar, pie_chart
...
class visualization_pipeline():
    def __init__(self,chart_type, title, file=None, df=None,cols_to_plot=None, is_file_path=False, watermark=None,dimensions=dict(height=400,width=730), 
                ...
                itemsizing='constant',num_col=None,barmode='stack',tickformat=dict(x='%b %d <br>%y',y1=".2s",y2=".2s"),
        ...
        if self.turn_to_time == False:
            if other == False:
                ...
                if self.groupby != None:
                    ...
                    self.axes_data['y1'] = self.axes_data['y1'].append(pd.Index(['Other']))
        else:
            if other == True:
                if self.groupby==None:
    ...
    def line_plot(self):
        ...
        if self.groupby == None:
            ...
            fig = simple_line_plot(df=self.df, title=self.title, annotations=self.annotations, show_legend=self.show_legend,
                                ...
                                remove_zero=self.remove_zero, custom_ticks=self.custom_ticks,font_family=self.font_family,font_color=self.font_color,directory=self.directory)
            self.fig = fig
            return fig
        else:
            # sorted_multi_line(df, title, save=False, combined_colors=combined_colors, mode='lines', col=None, sort_col=None, sort_list=True, area=False,
            ...
            #          tickformat=None, dtick=None, decimals=True,decimal_places=1,dimensions=dict(width=730,height=400),connectgaps=True):
    ...
    def bar_plot(self):
        ...
        if self.groupby == None:
            ...
            fig = sorted_bar_chart(df=self.df, title=self.title,col=self.num_col, sort_col=self.groupby,legend_orientation=self.legend_orientation,
                                    ...
                                    dimensions=self.dimensions,descending=self.descending,traceorder=self.traceorder,tickangle=self.tickangle,combined_colors=self.colors,
    ...
    def pie_plot(self):
        ...
        fig = pie_chart(df=self.df, sum_col=self.num_col, index_col=self.groupby,title=self.title,legend_placement=self.legend_placement,
                        ...
                        line_width=self.line_width,font_family=self.font_family,font_color=self.font_color,directory=self.directory)
        self.fig = fig
        return fig
